...
func ErrorFrom(err error) ErrorResponse {
	...
	case strings.Contains(msg, "requires") || strings.Contains(msg, "required") || strings.Contains(msg, "must be") || strings.Contains(msg, "must not be") || strings.Contains(msg, "can only be"):
		if body.Code == "command_failed" {
